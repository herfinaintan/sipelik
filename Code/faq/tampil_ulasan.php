...
$output='';
$query_ulasan =mysqli_query($con,"SELECT * FROM ulasan JOIN data_kosan ON ulasan.id_kosan=data_kosan.id_kosan WHERE parent_id_ulasan = '0' ORDER BY id_ulasan DESC");

while ($data_ulasan=mysqli_fetch_assoc($query_ulasan)) {
  $output .= '
    <div class="card" style="border:0">
    <h4 style="float: left;">'.$data_ulasan["nama_kosan"].'<small style="float: right;"> Posted on <i>'.$data_ulasan["tanggal"].'</i></small></h4>
    <div class="card card-body" style="float: left;margin-bottom:20px;">
      <p style="font-size:1.3rem"><b>'.$data_ulasan['nama_pengirim'].'</b></p>
...
{
  $output='';
  $query_ulasan1 ="SELECT * FROM ulasan JOIN data_kosan ON ulasan.id_kosan=data_kosan.id_kosan WHERE parent_id_ulasan=?";
