<?php
	...
	if (isset($_POST['input_paket'])) {
		...
		$query=mysqli_query($con,"INSERT INTO data_paket(nama_paket,jenis_paket,keterangan,harga_total,gambar_paket,jumlah_tersedia, sudah_diisi,status_paket) VALUES ('$nama_paket','$jenis_paket','$keterangan','$harga_total','$gambar','$jumlah_tersedia','$sudah_diisi','0')");
		if ($query) {
			echo '<script language="javascript">
	...
	if (isset($_POST['update_paket'])) {
		...
		$query = mysqli_query($con,"UPDATE data_paket SET nama_paket='$nama_paket', jenis_paket='$jenis_paket', keterangan='$keterangan', harga_total='$harga_total', jumlah_tersedia='$jumlah_tersedia', sudah_diisi='$sudah_diisi' WHERE id_paket='$id_paket'");
		if ($query) {
			echo '<script language="javascript">
	...
	if (isset($_POST['konfirmasi'])) {
		...
		$jumlah_transfer = $data['uang_muka'];
	...
	if (isset($_POST['confirm_pesanan'])) {
        ...
        $query1=mysqli_query($con, "UPDATE data_paket JOIN pesanan ON data_paket.id_paket=pesanan.id_paket SET sudah_diisi='++1' WHERE id_pesanan='$idp'");
        if ($query) {
	            echo '<script language="javascript">
	...
	if (isset($_POST['confirm_kosan'])) {
		$idk=$_POST['idk'];
        $query=mysqli_query($con,"UPDATE data_kosan SET status_kosan='1' WHERE id_kosan='$idk'");
        if ($query) {
        	if($_SESSION['level']=='admin'){
	            echo '<script language="javascript">
	              alert ("Pembayaran Dikonfirmasi!");
    ...
    if (isset($_POST['refuse_kosan'])) {
    	$idk=$_POST['idk'];
        $query=mysqli_query($con,"UPDATE data_kosan SET status_kosan='2' WHERE id_kosan='$idk'");
        if ($query) {
        	if($_SESSION['level']=='admin'){
	            echo '<script language="javascript">
	              alert ("Pembayaran Ditolak!");
